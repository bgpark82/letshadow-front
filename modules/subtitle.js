...
this.mmooc.youtube = function() {
	 ...
	 * 2. player 불러오기
	 * 3. 변수 설정
	 * @param {*} transcriptId videoTranscriptLm0m4VtZ3Us
	 * @param {*} language no
	 * @param {*} name bokm&aring;l
	 */
	function transcript(transcriptId, language, name)
	{
		var transcriptId = transcriptId;
		var videoId = transcriptId.split(transcriptIdPrefix)[1]; // Lm0m4VtZ3Us

		var href = hrefPrefix + videoId; // https://video.google.com/timedtext?v=Lm0m4VtZ3Us
		if(language != "")
		{
		...
		APIReady : function ()
		{
			if(!initialized)
			{
				$(".mmocVideoTranscript" ).each(function( i ) {
					var language = $(this).data('language'); // no
					var name = $(this).data('name'); // bokm&aring;l
					var oTranscript = new transcript(this.id, language, name); // 자막 xml 호출, player 호출
					oTranscript.getTranscript(); // 자막 xml -> 자막 dom
